...
class ContactAPITest(APITestCase):
    ...
    def test_list_contacts_excludes_master_and_ownerless_records_for_non_master(self):
        master = User.objects.create_user(username='masteruser', password='testpass123')
        ...
        self.assertNotIn('Contato do Master', names)
        self.assertNotIn('Contato Sem Dono', names)
    ...
    def test_master_default_scope_only_returns_own_contacts(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['id'], own_contact.id)
